...
export const NewEntryModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const { submit, isPending, error, setError } = useNewEntry(onClose)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFieldErrors({})
    try {
      await submit(header.trim(), body.trim())
    } catch {
      setError('ENCRYPTION_FAILED')
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5" style={{ padding: '1.5rem' }}>
          <TerminalField
            label="ENTRY_TITLE"
            value={header}
            onChange={(v) => {
